...
public sealed class AiBatchHubService : IAsyncDisposable
{
    private readonly IAccessTokenProvider _accessTokenProvider;
    private readonly ApiConfig _apiConfig;
    private readonly ILogger<AiBatchHubService> _logger;
    private HubConnection? _connection;
    private readonly SemaphoreSlim _connectLock = new(1, 1);
    private Guid? _joinedJobId;

    public AiBatchHubService(
        IAccessTokenProvider accessTokenProvider,
        ApiConfig apiConfig,
        ILogger<AiBatchHubService> logger)
    {
    ...
    public event Action<AiBatchJobDto>? BatchUpdated;
    ...
    private async Task EnsureConnectedAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var hubUrl = new Uri(new Uri(_apiConfig.Url), AiBatchHubClient.HubPath);
            ...
            _connection.On<AiBatchJobDto>(AiBatchHubClient.BatchUpdatedMethod, dto =>
            {
                BatchUpdated?.Invoke(dto);
            });
        ...
        {
            _logger.LogWarning(ex, "Failed to connect to AI batch hub");
        }
        finally
        {
